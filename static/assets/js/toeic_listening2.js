...
var letterlist=["A","B","C","D"];	
	...
	var audiofile="";
	var scriptlist=[];
	var questionaudiolist = [];
	var questionaudiolistb = [];
...
var nextquestion = questionlist[x];
var nextaudio=questionaudiolist[x];
	var choice1 = "";
			var  choice2 = "";
			var  choice3 = "";
	...
	questionlistb.push(nextquestion);
	questionaudiolistb.push(nextaudio);
...
txt=txt+  'Question '+ (n+1) + '&nbsp;&nbsp;<br><span id="result_' + n + '"><img src="../IELTS/blank.gif" style="border:0" alt="" /></span><span id="feedback'+n+'"></span>';
	txt=txt +"<br><div id='choices'>";
	 ...
	 for(j=0;j<numberofchoices;) {
		...
		txt=txt+ '<input type="radio" name="choices' + n + '" value="'+j+'" onClick="submitAnswer('+n+','+j+')"><label id="label_' + n + '_' + j + '" for="answer_' + n + '_' + j + '">&nbsp;<a   class="achoice" onClick="submitAnswer('+n+','+j+')" id="'+n + '_' + j + '">'  + letterlist[j] + '</a></label> <br>';
...
document.getElementById("score").innerHTML='<span id="score">&nbsp;&nbsp;&nbsp;Score: '+score+ '/'+ n +'</span>';
 ...
 {
answeredlist.push(0);
questionlist.push(x[i].getElementsByTagName("question")[0].childNodes[0].nodeValue)  ;	
 questionaudiolist.push(x[i].getElementsByTagName("question")[0].getAttribute("audio")); 
...
function seeScript(){
	document.getElementById("script3").innerHTML= "<BR />" +  questionlistb [questionnumber] + "<BR />" + letterlist[0] + ". " + textlistb [numberofchoices*questionnumber] + "<BR />" + letterlist[1] + ". " + textlistb [(numberofchoices*questionnumber)+1]  + "<BR />" + letterlist[2] + ". " + textlistb [(numberofchoices*questionnumber)+2]; 
...
audioitem="audio/" + questionaudiolistb[questionnumber];
playlist.push(audioitem);		
 for (i=0;i<numberofchoices;i++){
audioitem="audio/" + letterlist[i] + ".mp3";
playlist.push(audioitem);		
audioitem="audio/" + audiolistb[(numberofchoices*questionnumber)+i];
playlist.push(audioitem);
	}
